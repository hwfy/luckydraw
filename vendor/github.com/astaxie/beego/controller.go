...
func (c *Controller) SaveToFile(fromfile, tofile string) (string, error) {
	file, h, err := c.Ctx.Request.FormFile(fromfile)
	if err != nil {
		return "", err
	}
	defer file.Close()
	err = os.MkdirAll(tofile, 0777)
	if err != nil {
		return "", err
	}
	f, err := os.OpenFile(tofile+h.Filename, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0666)
	if err != nil {
		return "", err
	}
	defer f.Close()
	io.Copy(f, file)
	return tofile + h.Filename, nil
}
...
func (c *Controller) CheckXSRFCookie() bool {
	...
	return c.Ctx.CheckXSRFCookie(BConfig.WebConfig.XSRFKey)
}
